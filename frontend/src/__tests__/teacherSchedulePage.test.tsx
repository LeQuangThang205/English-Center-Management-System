...
function renderTeacherSchedule() {
  return render(
    <MemoryRouter initialEntries={['/teacher/schedule']}>
      <AuthProvider>
        <AppRoutes />
      </AuthProvider>
    </MemoryRouter>,
...
describe('TeacherSchedulePage', () => {
  ...
  it('renders page header with title and description', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Lịch dạy' })).toBeTruthy();
    expect(await screen.findByText('Lịch dạy của bạn trong khoảng thời gian đã chọn.')).toBeTruthy();
  });

  it('calls GET /api/schedules with from and to params', async () => {
    authStorage.setSession('jwt.teacher', teacherUser);
  ...
  it('shows empty state when teacher has no schedules in range', async () => {
    authStorage.setSession('jwt.teacher', teacherUser);
    vi.stubGlobal('fetch', createFetchMock(schedulesRouter([])));

    renderTeacherSchedule();

    expect(await screen.findByText('Chưa có lịch dạy')).toBeTruthy();
    expect(screen.getByText('Không có lịch dạy nào trong khoảng thời gian này.')).toBeTruthy();
  });
  ...
  it('shows error state and refetches when retrying', async () => {
    ...
    expect(await screen.findByText('Không thể tải lịch dạy')).toBeTruthy();
  ...
  it('shows loading skeleton while fetching', async () => {
    ...
    expect(screen.getByRole('status', { name: 'Đang tải lịch dạy' })).toBeTruthy();
    ...
    await screen.findByText('IELTS 6.0-01');
    expect(screen.queryByRole('status', { name: 'Đang tải lịch dạy' })).toBeNull();
  });

  it('renders TeacherSchedulePage for /teacher/schedule instead of PlaceholderPage', async () => {
    authStorage.setSession('jwt.teacher', teacherUser);
    vi.stubGlobal('fetch', createFetchMock(schedulesRouter([scheduleA])));
    ...
    expect(screen.queryByText('Lịch dạy chưa sẵn sàng')).toBeNull();
  });
